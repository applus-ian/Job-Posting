...
export function ListView({ interviews }: ListViewProps) {
  ...
  return (
    <div className="h-full flex flex-col p-2 sm:p-4">
      ...
      <div
        className="hidden sm:block divide-y overflow-y-auto max-w-4xl mx-auto w-full"
        style={{ height: "calc(100% - 42px)" }}
      >
        {sortedInterviews.map((interview, idx) => (
          <div
            key={idx}
            className="grid md:grid-cols-12 sm:grid-cols-6 py-3 px-4 items-center hover:bg-gray-50 text-sm"
          >
      ...
      <div className="sm:hidden divide-y overflow-y-auto h-full max-w-md mx-auto w-full">
        ...
        {sortedInterviews.length === 0 && (
          <div className="py-8 text-center text-gray-500">No interviews scheduled.</div>
        )}
      </div>
